...
public class HomeUI : MonoBehaviour
{
    ...
    private TMP_FontAsset tmpFontAsset; // 运行时创建的TMP字体资源

    void Start()
    {
        // 如果提供了传统Font，尝试创建对应的TMP_FontAsset以供TextMeshPro使用
        if (fontToUse != null)
        {
    ...
    public GameObject NewButton(Vector2 anchor, Vector2 buttonPosition, Vector2 size, string buttonName = "NewButton", string text = "Text", float scale = 2f)
    {
        ...
        // 为子Object添加TextMeshProUGUI组件并设置
    ...
    void RenderHome()
    {
        ...
        // 通过TextMeshProUGUI组件设置颜色和字体样式
        TextMeshProUGUI titleComp = titleText.GetComponent<TextMeshProUGUI>();
        if (titleComp != null)
        {
            titleComp.color = Color.black;   // 设置字体颜色
            titleComp.fontStyle = FontStyles.Bold; // 设置为粗体
        }
    ...
    void RenderSavasList()
    {
        ...
        if (textComp != null)
        {
            textComp.color = Color.black;   // 设置字体颜色
            textComp.fontStyle = FontStyles.Bold; // 设置为粗体
        }
        ...
        // 在 HomeUI 的 RenderSavasList 方法中，设置 dynamicSaveListUI 的 homeUI 字段
        ...
        dynamicSaveListUI.homeUI = this; // 新增：传递自身引用
    ...
    void DestorySavasList()
    {
        ...
        // 销毁 DynamicSaveListUI 创建的 UI 并移除组件
...
public class DynamicSaveListUI : MonoBehaviour
{
    ...
    public TMP_FontAsset tmpFontAsset; // 从HomeUI传入的字体
    public HomeUI homeUI; // 对于 HomeUI 的引用
    ...
    List<SaveData> LoadSaveData()
    {
        ...
        {
            new SaveData { saveName = "存档点1", saveTime = "2023-10-01 12:30", description = "He110 wOrLd?", screenshotPath = "Save1_Icon.png" },
            new SaveData { saveName = "自动存档", saveTime = "2023-10-01 13:45", description = "114???", screenshotPath = "ButtonImage.png" },
            new SaveData { saveName = "最终Boss前", saveTime = "2023-10-02 15:20", description = "啊锟斤拷烫烫烫！", screenshotPath = "ButtonImage.png" }
        };
    }
    ...
    void CreateScreenshotArea(GameObject parent, SaveData saveData)
    {
        ...
        raw.color = Color.white; // 默认背景

        // 尝试通过SaveManager加载存档icon
    ...
    public void DestroyUI()
    {
        ...
        // 销毁创建的UI对象（scrollView包含viewport和content）
        if (scrollView != null)
        {
            Destroy(scrollView);
            scrollView = null;
